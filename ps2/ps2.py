...
class RectangularRoom(object):
    ...
    def __init__(self, width, height):
        ...
        self.num_cleaned = 0
        self.room = [[0 for _ in range(width)] for _ in range(height)]
    ...
    def cleanTileAtPosition(self, pos):
        ...
        self.num_cleaned += not self.room[y][x]
        self.room[y][x] += 1
    ...
    def isPositionInRoom(self, pos):
        ...
        """
        return 0 <= pos.getX() < self.w and 0 <= pos.getY() < self.h
...
class StandardRobot(Robot):
    ...
    def updatePositionAndClean(self):
        ...
        if self.room.isPositionInRoom(p):
            ...
            while not self.room.isPositionInRoom(p):
                self.direction = random.randint(0, 359)
                p = self.position.getNewPosition(self.direction, self.speed)
...
def runSimulation(num_robots, speed, width, height, min_coverage, num_trials,
    ...
    for _ in range(num_trials):
        ...
        num_steps, num_tiles = 0, room.getNumTiles()
        while room.getNumCleanedTiles() / num_tiles < min_coverage:
            num_steps += 1
